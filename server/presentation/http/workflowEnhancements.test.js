...
test("real tournament routes persist random-every-round setup and retain manual pairings on list reload", async () => {
  ...
  registerTournamentRoutes({ app, getActorUser: () => ({ username: "captain" }), actorHasPermission: () => true, PERMISSIONS: { MANAGE_TOURNAMENTS: "manage" }, tournamentGateway: gateway, buildTournament, buildTournamentDataMaps: async () => ({ matchesByTournamentId: new Map([[1, matchRows]]), registrationsByTournamentId: new Map([[1, registrations]]), roundsByTournamentId: new Map(), scoresByTournamentId: new Map() }), memberDirectoryGateway: {}, TOURNAMENT_TEMPLATE_OPTIONS: [{ key: "random-rounds", label: "Random rounds", tournamentType: "head-to-head", format: "knockout", roundType: "portsmouth", capabilities: { randomiseEveryRound: true } }], TOURNAMENT_TYPE_OPTIONS: [{ value: "head-to-head" }], toUtcDateString: () => "2030-01-01", chooseRandomIndex: () => 0, getUtcTimestampParts: () => ["2030-01-01", "10:00:00"] });
  const response = () => ({ status(code) { this.code = code; return this; }, json(body) { this.body = body; return this; } });
  const created = response();
  await handlers.get("POST /api/tournaments")({ body: { name: "Test", tournamentType: "head-to-head", registrationStartDate: "2026-01-01", registrationEndDate: "2026-01-02", templateKey: "random-rounds" } }, created);

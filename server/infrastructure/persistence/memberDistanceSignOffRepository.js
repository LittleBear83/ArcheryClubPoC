export function createMemberDistanceSignOffRepository(
  ...
  let listDistanceSignOffsByUsername;
  let upsertDistanceSignOff;

  function getListDistanceSignOffsByUsername() {
    if (!listDistanceSignOffsByUsername) {
      listDistanceSignOffsByUsername = db.prepare(`
        SELECT
    ...
    return listDistanceSignOffsByUsername;
  }

  function getUpsertDistanceSignOff() {
    if (!upsertDistanceSignOff) {
      upsertDistanceSignOff = db.prepare(`
  ...
  function listByUsername(username) {
    return getListDistanceSignOffsByUsername()
      .all(username)
      .map(buildDistanceSignOff);
  }
  ...
  return {
    ...
    upsert(signOff) {
      getUpsertDistanceSignOff().run(signOff);
    },
  };
}
